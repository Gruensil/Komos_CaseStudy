...
@Pipe({name: 'accountListFilter'})
export class AccountListFilter implements PipeTransform{
...
return items.filter(item => 					item.balance.toString().toLowerCase().indexOf(filterBy['*'].toLowerCase()) !== -1||
